...
function DashboardAdmin() {
  ...
  const recentManagers = [...stats.managers]
    .sort((a, b) => b.ID_manager - a.ID_manager)
    .slice(0, 5);
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      {/* 🔥 BOUTON AJOUT MANAGER */}
      <section className="px-6">
        <Link to="/AjoutManager">
      ...
      <section className="p-6">
        <h2 className="text-xl font-bold mb-4">Managers récents</h2>
            ...
            <tbody>
              {recentManagers.length > 0 ? (
                recentManagers.map((m) => (
                  <tr key={m.ID_manager} className="border-b hover:bg-gray-50">

                    <td className="px-4 py-2">{m.Nom}</td>
                    <td className="px-4 py-2">{m.Prenom}</td>
                    <td className="px-4 py-2 hidden md:table-cell">{m.Telephone}</td>
                    <td className="px-4 py-2 hidden md:table-cell">{m.ID_boutique}</td>

                    <td className="px-4 py-2 space-x-2">
                       {/* ✅ EDITER */}
                      <Link to={`/EditerManager/${m.ID_manager}`}>
                      ...
                      {/* ✅ DELETE */}
                      <Link to={`/DeleteManager/${m.ID_manager}`}>
